...
namespace bullet_3da
{
		...
		Entity(/*scene*/)
	{
		...
		shared_ptr < Shape > wheel_shape(new Cylinder_Shape(3.75f, 3.75f, 1.3f));

		// wheel r1
		shared_ptr<Rigid_Body> wheel_rb_r1(new Dynamic_Rigid_Body(wheel_shape, 10.f));
		wheel_rb_r1->get()->getWorldTransform().setOrigin(btVector3(position.x - 7.f, position.y - 8.8f, position.z - 3.f));
		...
		wheel_rb_r2->get()->getWorldTransform().setOrigin(btVector3(position.x - 7.f, position.y - 3.f, position.z - 16.f));
		...
		wheel_rb_l1->get()->getWorldTransform().setOrigin(btVector3(position.x + 7.f, position.y - 8.8f, position.z - 3.f));
		...
		turret_constraint = new btHingeConstraint(*turret_rb->get(), *body_rb->get(), btVector3(0.f, -2.25f, 0.f), btVector3(0.f, 4.f, 0.f), btVector3(0.f, -6.5f, 0.f), btVector3(0.f, 0.f, 0.f));
		turret_constraint->enableAngularMotor(true, 0.f, 10.f);
		scene->get_physics_world()->addConstraints(turret_constraint);


		wheel_r1_constraint = new btHingeConstraint(*wheel_rb_r1->get(), *body_rb->get(), btVector3(0.f, 0.f, 0.f), btVector3(-7.f, -8.8f, -3.f), btVector3(0.f, 0.f, -1.f), btVector3(0.f, 0.f, 0.f));
		wheel_r1_constraint->enableAngularMotor(true, 0.f, 10.f);
		scene->get_physics_world()->addConstraints(wheel_r1_constraint);

		/*wheel_r2_constraint = new btHingeConstraint(*wheel_rb_r2->get(), *body_rb->get(), btVector3(0.f, 0.f, 0.f), btVector3(-7.f, -9.5f, -3.f), btVector3(0.f, 0.f, -1.f), btVector3(0.f, 0.f, 0.f));
		wheel_r2_constraint->enableAngularMotor(true, 0.f, 10.f);
		scene->get_physics_world()->addConstraints(wheel_r2_constraint);*/

		/*wheel_l1_constraint = new btHingeConstraint(*wheel_rb_l1->get(), *body_rb->get(), btVector3(0.f, 0.f, 0.f), btVector3(7.f, -3.f, -3.f), btVector3(0.f, 0.f, -1.f), btVector3(0.f, 0.f, 0.f));
